...
#	L.P.J.Perera (pjayendra101@gmail.com)
...
df1 = pd.read_csv("F:\HackStatFinal\diamonds.csv")
print(df1)
...
#there is lots of outliers also...and all categorical variables are not normal,
#and there are ordinal variables then we have to go for non parametric approach

    #Histogram
    #these are printed from there frequencies
    #clarity,color and cut are not normally skewed
    #clarity--> left skewd = positive skewed
    #color and cut --->negative skewed = right skewed
...
sns.distplot(traindf["depth"],bins=100)
        ...
        #test for normality
k,p=stats.mstats.normaltest(traindf["depth"])
k
p    #k=>z-score    p=>p-value
...
k,p=stats.mstats.normaltest(traindf["x"])

sns.distplot(traindf["y"],bins=100)
k,p=stats.mstats.normaltest(traindf["y"])

sns.distplot(traindf["z"],bins=100)
k,p=stats.mstats.normaltest(traindf["z"])
    # here x,y,z are related with table variable.not with y. 
    #so we need to compare it with table variable

    #correlation

correlations=traindf[['x','y','z','depth','price','carat','table']].corr()  
...
ax.set_xticks(ticks)
ax.set_yticks(ticks)
ax.set_xticklabels(['x','y','z','depth','price','carat','table'])
ax.set_yticklabels(['x','y','z','depth','price','carat','table'])
plt.show()

    #plot scatter plot
    #plot x variable vs depth variable
fig = plt.figure()
plt.scatter(x=traindf['x'],y=traindf['depth'])
fig.suptitle('Relationship between x and depth variable', fontsize=20)
plt.xlabel('x', fontsize=18)
plt.ylabel('depth', fontsize=16)

    #plot y variable vs depth variable
fig = plt.figure()
plt.scatter(x=traindf['y'],y=traindf['depth'])
fig.suptitle('Relationship between y and depth variable', fontsize=20)
plt.xlabel('y', fontsize=18)
plt.ylabel('depth', fontsize=16)

    #plot z variable vs depth variable
fig = plt.figure()
plt.scatter(x=traindf['z'],y=traindf['depth'])
fig.suptitle('Relationship between z and depth variable', fontsize=20)
